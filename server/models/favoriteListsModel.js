...
//-----Get list by id and user_id
  ...
  const { rows } = await db.query('SELECT * FROM "Favourite_lists" WHERE id = $1 AND user_id = $2',[listId, userId]);
        return rows[0];
    };

    // Mark list as shared with a UUID
    const shareFavoriteList = async (listId, shareUuid) => {
        const { rows } = await db.query('UPDATE "Favourite_lists" SET is_shared = true, share_uuid = $1 WHERE id = $2 RETURNING *',[shareUuid, listId]);
        return rows[0];
    };

    const getListByShareUuid = async (uuid) => {
        const { rows } = await db.query('SELECT * FROM "Favourite_lists" WHERE share_uuid = $1', [uuid]);
        return rows[0];
    };
